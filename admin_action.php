...
<div id="alertmsg">
  <?php if(!empty($_GET['ac'])){
	...
	$ac=$_GET['ac'];
	if($ac=='delete'){
		$db->delete("delete from ".TABLE_PREFIX."message where id=".$id);
		echo "留言已删除，3秒后返回，请稍候……";
	}elseif($ac=='settop'){
		$db->update("update ".TABLE_PREFIX."message set settop=1 where id=".$id);
		echo "留言已置顶，3秒后返回，请稍候……";
	}elseif($ac=='unsettop'){
		$db->update("update ".TABLE_PREFIX."message set settop=0 where id=".$id);
		echo "已取消置顶，3秒后返回，请稍候……";
	}elseif($ac=='setshow'){
		$db->update("update ".TABLE_PREFIX."message set ifshow=1 where id=".$id);
		echo "留言已显示，3秒后返回，请稍候……";
	}elseif($ac=='unshow'){
		$db->update("update ".TABLE_PREFIX."message set ifshow=0 where id=".$id);
