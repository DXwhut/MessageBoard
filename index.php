...
<body onload="i=0">
<?php if(!defined('MCBOOKINSTALLED')){?>
<div id="alertmsg"></div>
<?php exit();}?>
<!--最外层主要区域开始-->
<div id="main">
...
<font style="color:#999;">于 <?php echo date("Y-m-d H:i:s",strtotime($rows["systime"]));?> 发表留言：</font>
...
if($ifauditing==1){ 
if($rows['ifshow']==0){?>
<a href="admin_action.php?ac=setshow&amp;id=<?php echo $rows['id'];?>&page=<?php echo $page;?>"><img src="images/setshow.gif" title="审核并显示" /></a>
<?php }else{?>
<a href="admin_action.php?ac=unshow&amp;id=<?php echo $rows['id'];?>&page=<?php echo $page;?>"><img src="images/unshow.gif" title="隐藏此留言" /></a>
<?php }}?>
<a href="javascript:if(confirm('确认删除此留言?'))location='admin_action.php?ac=delete&amp;id=<?php echo $rows['id'];?>&page=<?php echo $page;?>'"><img src="images/icon_del.gif" title="删除此留言" /></a>
<a href="edit.php?id=<?php echo $rows['id'];?>&page=<?php echo $page;?>"><img src="images/icon_rn.gif" title="编辑/回复此留言" /></a>
<?php if($rows['settop']==0){?>
<a href="admin_action.php?ac=settop&amp;id=<?php echo $rows['id'];?>&page=<?php echo $page;?>"><img src="images/settop.gif" title="置顶留言" /></a>
<?php }else{?>
<a href="admin_action.php?ac=unsettop&amp;id=<?php echo $rows['id'];?>&page=<?php echo $page;?>"><img src="images/unsettop.gif" title="取消置顶" /></a>
<?php }}?>
</span>
<span class="rightarea">
<?php if(!empty($rows['email'])){?>
<a href="mailto:<?php echo $rows['email']?>"><img src="images/email.gif" title="点击用OutLook发送邮件至：<?php echo $rows['email']?>"></a>
<?php }?><?php if(!empty($_SESSION['admin_pass'])){?>
<img src="images/ip.gif" title="留言者IP：<?php echo $rows['userip'];?>"> 
<?php }?>
</span></h2>
<div class="content">
<?php
...
<label for="username">昵称：</label><input type="text" id="username" name="username" value="" />*<br />
<label for="email">Email：</label><input type="text" id="email" name="email" value="" /><br />
<label for="content">内容：</label><textarea id="content" name="content"></textarea>*<br />
<label for="content">　</label><span>提交前请按Ctrl+C保存留言内容，以免程序出错而丢失！
...
<label for="unum">验证码：</label><input name="unum" type="text" id="unum" size="10">* <img src="include/randnum.php" title="点击刷新" style="cursor:pointer" onclick="this.src='include/randnum.php'">
<br />
